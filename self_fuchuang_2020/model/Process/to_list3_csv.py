...
class PreProcess():
    ...
    def getGraph(self):
        ...
        print([i[0] for i in G.nodes.data()])
        G_mat = nx.adjacency_matrix(G)
        print(type(G_mat))
        print(G_mat.toarray())
        with open('data/list3_G.csv','w',newline='') as f3:
    ...
    def getFlow(self):
        ...
        for month in (3,4,5,6):
            for day in range(1, 30):
                ...
                print('month.day(dayprop):', month,day,dayprop)
                for hour in range(6, 23):
                    for minute in range(0, 2):
                        ...
                        for row in a:
                            st = 100 * float(row[0][3:]) + float(row[1][3:])
                            already_in_stations.append(st)
                            x_train_dict[st] = row[2]
                            # print(row[2])
        ...
        with open('data/list3_flow.csv','w',newline='') as f3:
            f3_csv = csv.writer(f3)
