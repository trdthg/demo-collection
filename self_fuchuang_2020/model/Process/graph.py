...
class Dfs():
    ...
    def getPassInfo(self, s, s2):
                ...
                if s in sta_list:
                    ...
                    if s2 in sta_list and g.getVertex(s2) == None:
                        ...
                        if index_of_s < index_of_s2:
                            return [sta_list[index_of_s: index_of_s2+1: 1], index_of_s2-index_of_s,[],0 [stationinfo_dict[s][0]] , 1.0]
                        else: 
                            return [sta_list[index_of_s: index_of_s2-1: -1], index_of_s-index_of_s2,[],0 [stationinfo_dict[s][0]] , 1.0]
        ...
        # 5.计算各路线权重
        min_stations = big_list[1].min()
        min_transfer = big_list[3].min()
        big_list[5] = ( 0 + ( -0.75 * (big_list[1])/min_stations ) + (-1.33 * (big_list[3]-min_transfer)) ).apply(np.exp)
        # 6. 计算权重比例
        maxest = big_list[5].sum()
        big_list[5] = big_list[5]/maxest
        return np.array(big_list).tolist()
