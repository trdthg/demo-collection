...
from tensorflow.keras.layers import Dense, SimpleRNN, Dropout, LSTM, GRU, Embedding
...
for i in range(7, len(x_train_1)):
    xarr = []
    xarr.append(x_train_1[i-7])
    xarr.append(x_train_1[i-6])
    xarr.append(x_train_1[i-5])
    xarr.append(x_train_1[i-4])
    xarr.append(x_train_1[i-3])
    xarr.append(x_train_1[i-2])
    xarr.append(x_train_1[i-1])
    x_train.append(xarr)
...
print(x_train.shape)
x_train = np.reshape(x_train, (x_train.shape[0],7, 194))
print(x_train.shape)

print("正在搭建模型")
model = tf.keras.Sequential([
    # Embedding(503, 64),
    LSTM(128, return_sequences=True),  # 两层都是RNN时,前一层要加上return_sequences=True
    Dropout(0.2),  # 随即扔掉一些神经元, 防止过拟合, 可以先设为0, 逐渐调大, 找到最优值
    LSTM(128, return_sequences=True),  # 两层都是RNN时,前一层要加上return_sequences=True
    Dropout(0.2),  # 随即扔掉一些神经元, 防止过拟合, 可以先设为0, 逐渐调大, 找到最优值
    LSTM(128, return_sequences=True),  # 两层都是RNN时,前一层要加上return_sequences=True
    Dropout(0.2),  # 随即扔掉一些神经元, 防止过拟合, 可以先设为0, 逐渐调大, 找到最优值
    LSTM(64),  # 两层都是RNN时,前一层要加上return_sequences=True
    Dropout(0.2),
    # LSTM(16),
    # Dropout(0.2),
    Dense(1),
