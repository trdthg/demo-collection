...
daily_flow_scaled = daily_flow
# print(daily_flow_scaled)

x_train, y_train = [], []

for anyday, dayprop, month, day, flow in zip(anydays, dayprop_onehot, np.array(df['month']), np.array(df['day']), np.array(daily_flow_scaled)):
    x_arr = list(dayprop) + [int(anyday), month, day]
    y_arr = flow[0]
