...
class DB():
    ...
    def __init__(self, DB, month=1, day=1):
        DB_USER = 'maker0'
        DB_PASS = 'Maker0000'
...
def main():
    global db
    # db = DB('library_flow', month=1, day=1)
    ...
    # for item in aaa:
    #     job(item)
    # tasks = [job(item) for item in aaa]
    # await asyncio.wait(tasks)
            # partial_work = partial(job,db = db)
            # re = pool.map(partial_work, aaa)
    pool = mp.Pool(processes=8) # 定义CPU核数量为n
    re = pool.map(job, aaa)
