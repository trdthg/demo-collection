...
class DB():
    ...
    def __init__(self, DB):
        ...
        DATABASE = DB
        try:
            self.connect_info = pymysql.connect(user=DB_USER, passwd=DB_PASS, host=DB_HOST, port=DB_PORT, db=DATABASE)
...
def main():
    ...
    re = pool.map(job, aaa)
        ...
        # break
    print("-------------- Done!--------------")
    print('耗时统计:', time.time()-time_start)
...
def job(df):
    ...
    data = ((key, float(temp[key][0]),float(temp[key][1]), float(temp[key][2]), float(temp[key][3]),time) for key in temp.keys())
    db.cursor.executemany(f"""INSERT INTO list1_{month} VALUES (%s,%s,%s,%s,%s,%s)""", data)
    data = ((key[0], key[1],float(temp2[key][0]), float(temp2[key][1]),time) for key in temp2.keys())
    db.cursor.executemany(f"""INSERT INTO list2_{month} VALUES (%s,%s,%s,%s,%s)""", data)
    data = ((key[0], key[1],float(temp3[key]),time) for key in temp3.keys())
    db.cursor.executemany(f"""INSERT INTO list3_{month} VALUES (%s,%s,%s,%s)""", data)
    data = ((key,float(temp4[key]),time) for key in temp4.keys())
    db.cursor.executemany(f"""INSERT INTO list4_{month} VALUES (%s,%s,%s)""", data)
    # for key in temp.keys():
